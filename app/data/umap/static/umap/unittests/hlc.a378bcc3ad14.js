...
const{expect}=pkg
import{HybridLogicalClock}from"../js/modules/sync/hlc.4ea17902b0e6.js"
describe('HybridLogicalClock',()=>{let clock
describe('#parse',()=>{it('should reject invalid values',()=>{clock=new HybridLogicalClock()
expect(()=>clock.parse('invalid')).to.throw()
expect(()=>clock.parse('123:456')).to.throw()
expect(()=>clock.parse('123:456:789:000')).to.throw()})
it('should parse correct values',()=>{clock=new HybridLogicalClock()
const result=clock.parse('1625097600000:42:abc-123')
expect(result).to.deep.equal({walltime:'1625097600000',nn:42,id:'abc-123',})})
it('should default to 0 for nn if none is provided',()=>{clock=new HybridLogicalClock()
const result=clock.parse('1625097600000::abc-123')
expect(result).to.deep.equal({walltime:'1625097600000',nn:0,id:'abc-123',})})})
describe('#serialize',()=>{it('should correctly serialize the clock',()=>{clock=new HybridLogicalClock(1625097600000,42,'abc-123')
expect(clock.serialize()).to.equal('1625097600000:42:abc-123')})})
describe('#tick',()=>{it('should increment walltime when current time is greater',()=>{const now=Date.now()
...
it('should increment nn when current time is not greater',()=>{const now=Date.now()
...
sinon.restore()})})
describe('#receive',()=>{it("should use current time when it's greater than both local and remote",()=>{const now=Date.now()
...
it('should increment nn when local and remote times are equal',()=>{const now=Date.now()
sinon.useFakeTimers(now)
...
expect(result.id).to.equal('local')})
it('should use remote time and increment nn when remote time is greater',()=>{const now=Date.now()
clock=new HybridLogicalClock(now-1000,5,'local')
const result=clock.receive(`${now}:7:remote`)
expect(result.walltime).to.be.least(now.toString())
expect(result.nn).to.equal(8)
expect(result.id).to.equal('local')})
it('should increment local nn when local time is greater',()=>{const now=Date.now()
clock=new HybridLogicalClock(now,5,'local')
const result=clock.receive(`${now - 1000}:7:remote`)
expect(result.walltime).to.be.least(now)
if(result.walltime>now){expect(result.nn).to.equal(5)}
else{expect(result.nn).to.equal(6)}
expect(result.id).to.equal('local')})})
it('should maintain causal order across multiple operations',()=>{const hlc=new HybridLogicalClock()
...
const uniqueIds=new Set([parsedEvent1.id,parsedEvent2.id,parsedEvent3.id,parsedEvent4.id,])
