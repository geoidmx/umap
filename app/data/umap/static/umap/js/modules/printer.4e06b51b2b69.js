...
export default class Printer{constructor(umap){this.umap=umap
this.dialog=this.umap.dialog}
build(){const[container,{format,mode,scale}]=Utils.loadTemplateWithRefs(`
      <div>
...
resetSize(){const map=this.umap._leafletMap
for(const name of Array.from(map._container.classList)){if(name.startsWith('print-')){map._container.classList.remove(name)}}
map._container.removeAttribute('style')
map.invalidateSize()}
resizeMap(){const form=this.dialog.collectFormData()
this.resetSize()
const map=this.umap._leafletMap
if(form.format&&form.mode){map._container.classList.add(`print-${form.format}`)
map._container.classList.add(`print-${form.mode}`)
map._container.style.width=`${form.scale}%`
map.invalidateSize()}}
open(action='print'){if(!this.container)this.build()
this.action=action
const acceptLabel=action==='print'?translate('Print'):translate('Download')
this.dialog.on('close',(event)=>{if(event.target.returnValue!=='accept'){this.resetSize()}},{once:true})
this.dialog.open({template:this.container,cancel:false,accept:acceptLabel}).then((form)=>this.onSubmit(form))
this.resizeMap()}
async onSubmit(form){this.umap.fire('dataloading',{id:'screenshot'})
if(this.action==='print'){const win=window.open('','_blank')
...
win.focus()}else{const screenshot=await this.umap.screenshot()
await screenshot.download({format:this.action,filename:Utils.slugify(this.umap.properties.name),})}
this.resetSize()
