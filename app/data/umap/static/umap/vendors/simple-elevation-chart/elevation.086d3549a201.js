function scale(max,min,num){const breathe=10
return(100*(num-min+breathe))/(max+breathe-min)||0}
function elevationColor(value){value=Math.round(Math.abs(value)/20)*20
const red=255-value*5
const green=125-value*10
const blue=value
return`rgb(${red} ${green} ${blue})`}
function draw(){const elem=document.getElementsByTagName('svg')[0]
...
let accX=0
const far=8
for(let i=0;i<length;i++){const farNext=dataEle[Math.min(i+far,length-far)%length]
...
elem.addEventListener('mouseover',(event)=>{const eleOver=new CustomEvent('chart:over',{detail:{element:event.target},})
